import mysql from "mysql2/promise";
import {
  DynamoDBClient,
  BatchGetItemCommand,
  QueryCommand,
  TransactWriteItemsCommand,
} from "@aws-sdk/client-dynamodb";

const dynamo = new DynamoDBClient({ region: process.env.REGION });

export const handler = async (event) => {
  console.log("Received event:", JSON.stringify(event, null, 2));

  // Support HTTP API (requestContext.http) and REST API (httpMethod/path)
  const method =
    event.requestContext?.http?.method || event.httpMethod || "UNKNOWN";
  const path =
    event.requestContext?.http?.path || event.path || "";

  const pathParams = event.pathParameters || {};
  const queryParams = event.queryStringParameters || {};

  // Safely parse JSON body (HTTP API often passes string)
  let body = {};
  try {
    body = event.body ? JSON.parse(event.body) : {};
  } catch {
    body = {};
  }

  // CORS preflight
  if (method === "OPTIONS") {
    return {
      statusCode: 200,
      headers: corsHeaders(),
      body: "",
    };
  }

  const conn = await mysql.createConnection({
    host: process.env.DB_HOST,
    user: process.env.DB_USER,
    password: process.env.DB_PASS, // ensure this env var is set
    database: process.env.DB_NAME,
  });

  try {
    // [5] GET /events  (must be before /event to avoid route collision)
    if (method === "GET" && path === "/events") {
      console.log("Getting all events");
      const [rows] = await conn.execute(`
        SELECT * FROM events
        ORDER BY start_at ASC
      `);
      return json(rows);
    }

    // [1] GET /event/{event_id}
    // Make the match stricter so "/events" does not hit here
    if (method === "GET" && (path === "/event" || path.startsWith("/event/"))) {
      const eventId = pathParams.event_id;
      console.log("Looking for event_id", eventId);

      if (!eventId) {
        return json({ message: "Missing path parameter: event_id" }, 400);
      }

      const [rows] = await conn.execute(
        "SELECT * FROM events WHERE event_id = ?",
        [eventId]
      );

      if (rows.length === 0) {
        return json({ message: "Event not found" }, 404);
      }

      return json(rows[0]);
    }

    // [2] GET /stats/{event_id}
    if (method === "GET" && (path === "/stats" || path.startsWith("/stats/"))) {
      const eventId = pathParams.event_id;
      console.log("Looking for stats for event_id", eventId);

      if (!eventId) {
        return json({ message: "Missing path parameter: event_id" }, 400);
      }

      const responses = ["Yes", "No"];
      const keys = responses.map((r) => ({
        pk: { S: `EVENT#${eventId}` },
        sk: { S: `RESPONSE#${r}` },
      }));

      const result = await dynamo.send(
        new BatchGetItemCommand({
          RequestItems: {
            "event-rsvp-responses": { Keys: keys },
          },
        })
      );

      const items = result.Responses?.["event-rsvp-responses"] || [];
      const counts = { Yes: 0, No: 0 };

      for (const item of items) {
        const resp = item.sk.S.split("#")[1]; // "Yes" or "No"
        counts[resp] = Number(item.count?.N || 0);
      }

      return json(counts);
    }

    // [3] POST /rsvp
    if (method === "POST" && path === "/rsvp") {
      const { event_id, full_name, email, response } = body;
      console.log("RSVP request:", { event_id, full_name, email, response });

      if (!event_id || !full_name || !email || !response) {
        return json({ message: "Missing required fields" }, 400);
      }

      const now = Date.now();

      try {
        await dynamo.send(
          new TransactWriteItemsCommand({
            TransactItems: [
              {
                Put: {
                  TableName: "event-rsvp-responses",
                  Item: {
                    pk: { S: `EVENT#${event_id}` },
                    sk: { S: `ATTENDEE#${email}` },
                    full_name: { S: full_name },
                    response: { S: response },
                    created_at: { N: String(now) },
                  },
                  // fix typo: attribute_not_exists
                  ConditionExpression:
                    "attribute_not_exists(pk) AND attribute_not_exists(sk)",
                },
              },
              {
                Update: {
                  TableName: "event-rsvp-responses",
                  Key: {
                    pk: { S: `EVENT#${event_id}` },
                    sk: { S: `RESPONSE#${response}` },
                  },
                  UpdateExpression: "ADD #count :one",
                  ExpressionAttributeNames: {
                    "#count": "count",
                  },
                  ExpressionAttributeValues: {
                    ":one": { N: "1" },
                  },
                },
              },
            ],
          })
        );

        return json({ message: "RSVP recorded!" }, 200);
      } catch (error) {
        if (
          error.name === "TransactionCanceledException" ||
          error.name === "ConditionalCheckFailedException"
        ) {
          return json(
            {
              message:
                "You have already RSVP'd for this event with this email!",
              code: "DUPLICATE_RSVP",
            },
            409
          );
        }

        console.log("DynamoDB error:", error);
        return json({ error: error.message }, 500);
      }
    }

    // [4] GET /attendees/{event_id}?response=Yes|No
    if (
      method === "GET" &&
      (path === "/attendees" || path.startsWith("/attendees/"))
    ) {
      const eventId = pathParams.event_id;
      const responseType = queryParams.response; // optional filter

      if (!eventId) {
        return json({ message: "Missing path parameter: event_id" }, 400);
      }

      const exprNames = {};
      const exprValues = {
        ":pk": { S: `EVENT#${eventId}` },
        ":prefix": { S: "ATTENDEE#" },
      };

      const queryInput = {
        TableName: "event-rsvp-responses",
        KeyConditionExpression: "pk = :pk AND begins_with(sk, :prefix)",
        ExpressionAttributeValues: exprValues,
      };

      // Optional filtering by response (use FilterExpression, not KeyCondition)
      if (responseType) {
        exprNames["#resp"] = "response";
        exprValues[":respVal"] = { S: responseType };
        queryInput.FilterExpression = "#resp = :respVal";
        queryInput.ExpressionAttributeNames = exprNames;
      }

      const result = await dynamo.send(new QueryCommand(queryInput));

      let attendees = (result.Items || []).map((item) => {
        const sk = item.sk.S; // "ATTENDEE#email@example.com"
        const email = sk.includes("#") ? sk.split("#")[1] : null;

        return {
          full_name: item.full_name?.S || null,
          email,
          response: item.response?.S || null,
          created_at: item.created_at?.N ? Number(item.created_at.N) : null,
        };
      });

      return json(attendees);
    }

    return json({ message: "Route Not Found" }, 404);
  } catch (error) {
    console.log("Error:", error);
    return json({ error: error.message }, 500);
  } finally {
    if (conn) await conn.end();
  }
};

// helper function for consistent JSON responses
function json(data, statusCode = 200) {
  return {
    statusCode,
    headers: corsHeaders(),
    body: JSON.stringify(data),
  };
}

function corsHeaders() {
  return {
    "Content-Type": "application/json",
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "OPTIONS, POST, GET, PUT, DELETE",
    "Access-Control-Allow-Headers":
      "Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,X-Requested-With",
    "Access-Control-Allow-Credentials": true,
  };
}
