...
async function submitRSVP(event, eventId) {
  ...
  try {
    ...
    // IMPORTANT: await openEventModal() so the DOM is rebuilt BEFORE calling loadEventStats(),
    // otherwise loadEventStats() may try to update elements that do not exist yet (null.textContent error).
    setTimeout(async () => {
...
async function loadEventStats(eventId) {
  try {
    ...
    // These IDs must exist in the modal HTML (added below in openEventModal()).
    // If they don't exist, getElementById() returns null and setting textContent crashes.
    const yesEl = document.getElementById(`yes-${eventId}`);
    const noEl = document.getElementById(`no-${eventId}`);

    // Defensive guard: prevents "Cannot set properties of null" if the modal is not open yet
    // or if the markup was changed.
...
async function openEventModal(eventId) {
  try {
    ...
    const banner =
      event.banner_url ||
      'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1200'; // Fallback banner image if none provided by the event record
    ...
    modalContent.innerHTML = `
      ...
      <div class="modal-body">
        ...
        <div class="section rsvp-section">
          ...
          <div class="stats-bar">
            <div class="stat-card yes-card">
              <!-- FIX: add an ID that matches loadEventStats(): yes-${eventId} -->
              <span class="stat-number" id="yes-${eventId}">${stats.Yes ?? 0}</span>
              <span class="stat-label">Yes Responses</span>
            </div>
            <div class="stat-card no-card">
              <!-- FIX: add an ID that matches loadEventStats(): no-${eventId} -->
    ...
    document.getElementById('modalContent').innerHTML = `<div class="error">Error loading event details: ${error.message}</div>`;
  }
}